...
ImportedModel::ImportedModel(const char* filePath) {
	...
	std::vector<float> nd = modelImporter.getNode();
	FaceId = modelImporter.getFaceId();
	numNode = nd.size();
	...
	for (int i = 0; i < (numNode / 3); i++) {
		node.push_back(glm::vec3(nd[i * 3], nd[i * 3 + 1], nd[i * 3 + 2]));
	}
...
int ImportedModel::getNodeNum() { return node.size(); }
...
std::vector<glm::vec3> ImportedModel::getNode() { return node; }
std::vector<int> ImportedModel::getFaceId() { return FaceId; }
// ---------------------------------------------------------------
...
std::vector<float> ModelImporter::getNode() { return vertVals; }
...
std::vector<int> ModelImporter::getFaceId() { return FaceId; }